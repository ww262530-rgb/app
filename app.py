...
st.title("📈 Python 自動化股市監控與 AI 決策系統 (FinMind 精確抗限流版)")
...
# 3. 核心修正：利用 FinMind API 免費免金鑰通道 (不擋 IP、價格 100% 真實精確)
@st.cache_data(ttl=300)  # 快取 5 分鐘，優化載入速度
def fetch_finmind_data(stock_id):
    """
    透過 FinMind API 獲取真實的台股歷史股價
    """
    url = "https://finmindtrade.com"
    
    # 計算時間起訖：為了能精準算出 60MA (季線)，我們一律多抓 5 個月前的資料
    ...
    try:
        response = requests.get(url, params=parameter, timeout=15)
        if response.status_code != 200:
            return pd.DataFrame(), f"台股 {stock_id}"
            
        json_data = response.json()
        if json_data.get("msg") != "success" or not json_data.get("data"):
            return pd.DataFrame(), f"台股 {stock_id}"
        ...
        # 建立標準 K 線格式
        df = pd.DataFrame()
        df['Date'] = pd.to_datetime(raw_df['date'])
        df['Open'] = pd.to_numeric(raw_df['open'])
        df['High'] = pd.to_numeric(raw_df['max'])
        df['Low'] = pd.to_numeric(raw_df['min'])
        df['Close'] = pd.to_numeric(raw_df['close'])
        df['Volume'] = pd.to_numeric(raw_df['Trading_Volume'])
        ...
        try:
            info_url = "https://finmindtrade.com"
            info_param = {"dataset": "TaiwanStockInfo"}
            info_res = requests.get(info_url, params=info_param, timeout=5).json()
            if info_res.get("data"):
                info_df = pd.DataFrame(info_res["data"])
                matched = info_df[info_df['stock_id'] == stock_id]
                if not matched.empty:
                    company_name = matched.iloc[0]['stock_name']
        except Exception:
            pass
...
try:
    ...
    with st.spinner('🔄 正在從 FinMind 金融大數據庫下載 100% 精確歷史數據...'):
        df, company_name = fetch_finmind_data(ticker_clean)
        
    if df.empty:
        st.error("🚨 無法取得真實股價數據。原因：股票代號輸入錯誤，或是 FinMind 伺服器忙碌中。")
        st.info("💡 提示：請輸入純數字（如 2330、0050、5483），不需加後綴。")
    else:
        # --- 5. 計算精確量化指標 ---
        ...
        with ai_col2:
            ...
            if close_price < ma20_now and ma5_now < ma20_now:
                ...
                st.caption("原因：個股跌破月線且週線月線呈現死亡交叉，空頭排列確認，適合反向或避險避空。")
            elif rsi_val > 75:
                st.warning("⚠️ **評估結果：可考慮右側左高放空**")
                st.caption("原因：技術面極度超買，正乖離過大，可待黑 K 確立後進場做空。")
            else:
                st.info("⚪ **評估結果：暫不建議放空**")
                st.caption("原因：目前股價仍受核心均線支持，或是處於箱型橫盤，放空極易被扎，建議多看少動。")

        # 欄位三：整合操作策略建議 (精準連動週線、月線動態價位)
        with ai_col3:
            ...
            if score >= 2:
                ...
                st.caption("指標交織，大盤或個股進入整理期。建議拉大買賣價位，切勿追漲殺跌。")
            else:
            ...
            # 依現價與均線位元，智慧轉換提示文字（解決買點停損比現價高引起的困惑）
            if close_price >= ma5_now:

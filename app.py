        ...
        try:
            ...
            if res.status_code == 200:
                ...
                if data.get("stat") == "OK" and "data" in data:
                    ...
                    df_list.append(df_month[["Date", "Volume", "Open", "High", "Low", "Close"]])
                    if "title" in data:
                        company_name = data["title"].split(" ") if len(data["title"].split(" ")) > 2 else company_name
        except Exception:
            continue
            
    # 若上市找不到，自動切換至櫃買中心（上櫃股票接口）
    if not df_list:
        for i in range(5, -1, -1):
            target_date = today - timedelta(days=i * 30)

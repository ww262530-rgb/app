...
try:
    ...
    with ai_col3:
        ...
        # --- ⚡ 智能調整：根據現價與均線關係自動轉換警示文字 ⚡ ---
        st.markdown("---")
        st.markdown("📐 **量化價位參考點**")
        
        # 買點動態提示
        if close_price >= ma5_now:
            st.markdown(f"🟢 **波段支撐買點 (5MA)**：`{ma5_now:.2f}` (現價已在線上，拉回不破可佈局)")
        else:
            st.markdown(f"⏳ **右側確認買點 (5MA)**：`{ma5_now:.2f}` (現價在線下，需等站回此價位再確認止跌)")
            
        # 停損點動態提示
        if close_price >= ma20_now:
            st.markdown(f"🔴 **建議保命停損 (20MA)**：`{ma20_now:.2f}` (跌破此價位必須嚴格出場)")
        else:
            st.markdown(f"🚨 **保命停損確認 (20MA)**：`{ma20_now:.2f}` (現價已全面跌破！目前不宜進場承接)")
            
        st.caption("💡 註：當買點與停損點皆高於現價，代表個股處於急跌或弱勢空頭排列，請靜待股價站回均線。")
